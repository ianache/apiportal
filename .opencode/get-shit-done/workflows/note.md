...
- **Global scope**: `D:/02-PERSONAL/01-PROJECTS/21-APIManager/.opencode/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `D:/02-PERSONAL/01-PROJECTS/21-APIManager/.opencode/notes/`)
...
2. Glob `D:/02-PERSONAL/01-PROJECTS/21-APIManager/.opencode/notes/*.md` (if directory exists) — global notes
...
Global (D:/02-PERSONAL/01-PROJECTS/21-APIManager/.opencode/notes/):
...
2. **No `.planning/`**: Falls back to global `D:/02-PERSONAL/01-PROJECTS/21-APIManager/.opencode/notes/` — works in any directory
