...
```bash
INIT=$(node "D:/02-PERSONAL/01-PROJECTS/21-APIManager/.opencode/get-shit-done/bin/gsd-tools.cjs" init map-codebase)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_MAPPER=$(node "D:/02-PERSONAL/01-PROJECTS/21-APIManager/.opencode/get-shit-done/bin/gsd-tools.cjs" agent-skills gsd-codebase-mapper 2>/dev/null)
```
...
```bash
node "D:/02-PERSONAL/01-PROJECTS/21-APIManager/.opencode/get-shit-done/bin/gsd-tools.cjs" commit "docs: map existing codebase" --files .planning/codebase/*.md
```
