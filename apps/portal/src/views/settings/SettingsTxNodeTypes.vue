  ...
  <Shell>
    <div class="p-8 max-w-7xl mx-auto w-full" style="font-family:'Inter',sans-serif;">
      ...
      <header class="flex justify-between items-center mb-8">
        <div>
          <h1 class="text-3xl font-bold" style="color:#1a1b1f;">Tx Node Types</h1>
          <p class="mt-1 text-sm" style="color:#717786;">
            Define reusable node types for the Transformation Designer.
          </p>
        </div>
        <button @click="openCreate"
          ...
          <span class="material-symbols-outlined" style="font-size:18px;">add</span>New Tx Node Type
        </button>
      </header>
      ...
      <div v-if="!store.entries.length"
        ...
        <div class="w-20 h-20 rounded-full flex items-center justify-center mb-6" style="background:#f4f3f8;">
          <span class="material-symbols-outlined text-4xl" style="color:#717786;">transform</span>
        </div>
        <h2 class="text-xl font-bold mb-2" style="color:#1a1b1f;">No tx node types yet</h2>
        <p class="mb-8 max-w-sm text-sm" style="color:#414755;">
          Create transformation node types to use as reusable building blocks in your data transformation flows.
        </p>
        <button @click="openCreate"
          class="py-2.5 px-6 rounded-xl text-sm font-semibold flex items-center gap-2 transition-opacity hover:opacity-90"
          style="background:#0058bc;color:#fff;box-shadow:0 4px 14px rgba(0,88,188,0.30);">
          <span class="material-symbols-outlined" style="font-size:18px;">add_circle</span>Create First Tx Node Type
        </button>
      </div>
        ...
        <div v-for="entry in store.entries" :key="entry.id" class="nt-card">
          ...
          <div class="flex items-start justify-between gap-3 mb-3">
            <div class="flex items-center gap-3 min-w-0">
              <div class="nt-icon" :style="{ background: (entry.color || '#0058bc') + '15' }">
                <span class="material-symbols-outlined" 
                  :style="{ fontSize: '20px', color: entry.color || '#0058bc', fontVariationSettings: `'FILL' 1` }">
                  {{ entry.icon || 'transform' }}
                </span>
              </div>
          ...
          <div v-if="entry.properties.length" class="space-y-1">
            <p class="text-xs font-bold uppercase tracking-wider mb-1.5" style="color:#a0a7b5;">Parameters</p>
            <div v-for="prop in entry.properties" :key="prop.name" class="flex items-center gap-2">
              ...
              <span class="px-1.5 py-0.5 rounded bg-slate-100 text-[8px] font-black uppercase tracking-tighter" :class="prop.nature === 'input' ? 'text-blue-600' : 'text-amber-600'">
                {{ prop.nature }}
              </span>
              <span v-if="prop.required" class="nt-prop-required">required</span>
            </div>
          </div>
          <p v-else class="text-xs" style="color:#c7c6d1;">No parameters defined</p>
        </div>
      </div>
    ...
    <div v-if="modal.open" class="fixed inset-0 z-[100] flex items-center justify-center p-6"
      ...
      <div class="rounded-3xl w-full max-w-2xl shadow-2xl flex flex-col" style="background:#fff;max-height:90vh;">

        <!-- Modal header -->
        <div class="flex items-start justify-between p-8 pb-4 flex-shrink-0">
          <div>
            <h2 class="text-2xl font-bold" style="color:#1a1b1f;">{{ modal.editing ? 'Edit Tx Node Type' : 'New Tx Node Type' }}</h2>
            <p class="text-sm mt-1" style="color:#717786;">Define a reusable transformation node type.</p>
          </div>
        ...
        <div class="overflow-y-auto px-8 pb-2 flex-1 min-h-0">
          ...
          <div class="space-y-4">
            ...
            <div>
              ...
              <input v-model="form.name" type="text" class="field-input" placeholder="e.g. JSON Transformer"
            ...
            <div class="grid grid-cols-2 gap-4">
              <div>
                <label class="field-label">Icon (Emoji or MD Icon)</label>
                <div class="flex gap-2">
                  <input v-model="form.icon" type="text" class="field-input" placeholder="e.g. 🔄 or transform" />
                  <div class="w-10 h-10 rounded-xl bg-slate-100 flex items-center justify-center shrink-0">
                    <span v-if="isEmoji(form.icon)" class="text-xl">{{ form.icon }}</span>
                    <span v-else class="material-symbols-outlined text-slate-600">{{ form.icon || 'transform' }}</span>
                  </div>
                </div>
              </div>
              <div>
                ...
                <div class="flex gap-2">
                  <input v-model="form.color" type="color" class="h-10 w-10 p-0 border-none bg-transparent shrink-0 cursor-pointer" />
                  <input v-model="form.color" type="text" class="field-input uppercase" placeholder="#0058BC" />
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              <textarea v-model="form.description" class="field-input field-textarea" rows="2"
                placeholder="What does this tx node type do?" />
            </div>

            <!-- Properties -->
            <div>
              <div class="flex items-center justify-between mb-2">
                <label class="field-label" style="margin:0;">Parameters</label>
                <button @click="addProp"
                  ...
                  style="color:#0058bc;">
                  <span class="material-symbols-outlined" style="font-size:14px;">add</span>Add Parameter
                </button>
              </div>

              <div v-if="!form.properties.length" class="text-xs text-center py-4 rounded-xl" style="color:#a0a7b5;background:#f4f3f8;">
                No parameters yet — click "Add Parameter" to define inputs or variables.
              </div>

              <div v-for="(prop, i) in form.properties" :key="i" class="prop-row items-center">
                <input v-model="prop.name" type="text" class="field-input prop-input" placeholder="Name" />
                ...
                <select v-model="prop.nature" class="field-input prop-select">
                  <option value="input">Input (Node Pin)</option>
                  <option value="variable">Variable (Editor)</option>
                </select>
                <label class="flex items-center gap-1 text-[10px] font-bold uppercase tracking-tighter flex-shrink-0" style="color:#414755;">
    ...
    <div v-if="deleteId" class="fixed inset-0 z-[100] flex items-center justify-center p-6"
      ...
      <div class="rounded-3xl w-full max-w-sm p-8 shadow-2xl text-center" style="background:#fff;">
        ...
        <h2 class="text-xl font-bold mb-2" style="color:#1a1b1f;">Delete tx node type?</h2>
...
import type { TxNodeTypeCatalogEntry, NodePropDef } from '../../stores/txNodeTypeCatalog';

const store = useTxNodeTypeCatalogStore();
...
// ── Form ──────────────────────────────────────────────────
interface PropForm { name: string; type: NodePropDef['type']; nature: NodePropDef['nature']; required: boolean; }
const emptyForm = () => ({ name: '', description: '', icon: 'transform', color: '#0058bc', properties: [] as PropForm[] });
...
function openEdit(entry: TxNodeTypeCatalogEntry) {
  Object.assign(form, {
    name: entry.name,
    description: entry.description ?? '',
    icon: entry.icon ?? 'transform',
    color: entry.color ?? '#0058bc',
...
function addProp() {
  form.properties.push({ name: '', type: 'string', nature: 'input', required: false });
}
...
.prop-select { width: 130px; flex-shrink: 0; }
</style>
